    ...
    <header className="w-full bg-white border-b-4 border-black sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          <div className="bg-neoyellow border-3 border-black p-2 font-black text-xl tracking-tighter uppercase neo-shadow-sm group-hover:translate-x-[-2px] group-hover:translate-y-[-2px] group-hover:shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] transition-all select-none rounded-[12px] flex items-center gap-1">
            <Sparkles size={20} className="fill-black text-black animate-pulse" /> Open <span className="bg-black text-white px-2 py-0.5 ml-1 rounded-[6px]">Guestbook</span>
          </div>
        </Link>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                href={item.path}
                className={`flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 border-3 border-black text-xs sm:text-sm font-extrabold uppercase tracking-tight transition-all duration-100 rounded-full ${
                  isActive
